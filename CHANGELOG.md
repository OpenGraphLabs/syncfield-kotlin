...
- **No on-device test coverage**: the legacy path has been validated by unit test and code review only — no Android 9 hardware is currently in the test lab. Production telemetry (`camera_ap_join_*_legacy` / `wifi_legacy_release_*` events emitted via `InstaLog`) is the canary; surface any OEM-specific anomalies through the host app's logging pipeline.
